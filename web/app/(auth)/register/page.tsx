...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setPending(true);
    try {
      const res = await fetch(`${API_BASE}/api/v1/authentication/user`, {
        method: "POST",
      ...
      setSuccess("Account created! Check your email for the activation link.");
    ...
    <div className="flex flex-1 items-center justify-center px-4 py-16">
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-surface-900 dark:text-white">Join EchoNet</h1>
          <p className="mt-1.5 text-sm text-surface-500 dark:text-surface-400">Create your account and start sharing</p>
        </div>
        ...
        <div className="rounded-2xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-7 shadow-sm">
          ...
          {success && (
            <div className="mb-5 rounded-xl border border-green-200 dark:border-green-900/50 bg-green-50 dark:bg-green-950/40 px-4 py-3 text-sm font-medium text-green-700 dark:text-green-400 flex items-center gap-2">
              <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
